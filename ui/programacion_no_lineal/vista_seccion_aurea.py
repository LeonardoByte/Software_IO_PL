...
from src.controller.controlador_no_lineal import ControladorNoLineal, OPCION_BISECCION
...
@ft.component
def VistaBiseccion(controlador: ControladorNoLineal):
    problema = controlador.problema_activo

    if problema is None:
        return sin_problema("Bisección", "Define y guarda un problema en «Ingresar PNL» primero.")

    respuesta: RespuestaNoLineal = controlador.resolver_NL(problema, OPCION_BISECCION)

    if respuesta is None:
        return sin_problema("Bisección", "Error al ejecutar el solucionador.")

    # ── cabecera y resultado ────────────────────────────────────────────────────
    header = encabezado_resultado(
        titulo="Bisección — Reducción del Intervalo",
        subtitulo=(
